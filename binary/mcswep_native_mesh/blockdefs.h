...
namespace mcmesh::blockdefs {
	...
	inline std::unordered_map<uint16_t, BlockDef>g_defs; inline std::vector<StateDef>g_states; inline std::vector<uint8_t>g_lightEmission, g_lightOpacity; inline VisualCatalog g_visual{}; inline AtlasParams g_atlas{}; inline AggregateCounts g_counts{}; inline std::array<uint8_t, 32>g_schemaHash{}, g_visualHash{}; inline bool g_loaded = false, g_sealed = false; inline std::shared_mutex g_mutex;
	...
	bool Parse(const void*, size_t); bool Seal(); void Unseal(); void Clear(); int SetBlockDefs(ILuaBase*); int SetLightDefs(ILuaBase*); int DebugBlockDef(ILuaBase*); int DebugStateDef(ILuaBase*);
}
