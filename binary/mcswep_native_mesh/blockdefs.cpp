...
namespace mcmesh::blockdefs {
    ...
    static bool ParseImpl(const void* data, size_t len) {
        ...
        catch (...) { return false; }for (uint32_t si = 0; si < h.states; ++si) { uint32_t stateId = 0; StateDef s; uint8_t reserved = 0; if (!r.Read(stateId) || !r.Read(s.blockId) || !r.Read(s.orient) || !r.Read(s.flags) || !r.Read(s.liquidKind) || !r.Read(s.liquidAmount) || !r.Read(reserved) || stateId != si || (s.flags & ~STATE_KNOWN_MASK) || s.liquidKind > 2 || s.liquidAmount > 8 || ((s.liquidKind == 0) != (s.liquidAmount == 0)) || reserved)return false; for (auto& plan : s.stairPlans)if (!r.Read(plan))return false; if (si == 0) { if (s.blockId || s.orient || s.flags || s.liquidKind || s.liquidAmount || std::any_of(s.stairPlans.begin(), s.stairPlans.end(), [](uint16_t p) {return p != 0; }))return false; } else { auto di = parsed.find(s.blockId); if (di == parsed.end() || !OrientEntry(&di->second, s.orient) || uint8_t(di->second.liquid) != s.liquidKind)return false; if ((s.flags & STATE_STAIRS) == 0 && std::any_of(s.stairPlans.begin(), s.stairPlans.end(), [](uint16_t p) {return p != 0; }))return false; if ((s.flags & STATE_STAIRS) && std::any_of(s.stairPlans.begin(), s.stairPlans.end(), [](uint16_t p) {return p == 0; }))return false; }states[si] = s; }VisualCatalog visualCatalog; if (!parseVisual(r, h, tiles, visualCatalog))return false; for (const auto& s : states)for (uint16_t plan : s.stairPlans)if (plan > visualCatalog.plans.size())return false;
    ...
    int SetLightDefs(ILuaBase* L) {
        unsigned int n=0;const char* p=L->GetString(1,&n);
        std::unique_lock<std::shared_mutex> lock(g_mutex);
        if(!p||!g_loaded||g_sealed||n!=g_states.size()*2u){L->PushBool(false);return 1;}
        std::vector<uint8_t> emission,opacity;
        try{emission.resize(g_states.size());opacity.resize(g_states.size());}catch(...){L->PushBool(false);return 1;}
        for(size_t i=0;i<g_states.size();++i){const uint8_t e=(uint8_t)p[i*2],o=(uint8_t)p[i*2+1];if(e>15||o>15){L->PushBool(false);return 1;}emission[i]=e;opacity[i]=o;}
        g_lightEmission=std::move(emission);g_lightOpacity=std::move(opacity);L->PushBool(true);return 1;
    }
    bool Seal() { std::unique_lock<std::shared_mutex>l(g_mutex); if (!g_loaded)return false; g_sealed = true; return true; }void Unseal() { std::unique_lock<std::shared_mutex>l(g_mutex); g_sealed = false; }void Clear() { std::unique_lock<std::shared_mutex>l(g_mutex); g_defs.clear(); g_states.clear(); g_lightEmission.clear(); g_lightOpacity.clear(); g_visual = {}; g_atlas = {}; g_counts = {}; g_schemaHash = {}; g_visualHash = {}; g_loaded = g_sealed = false; }int SetBlockDefs(ILuaBase* L) { unsigned int n = 0; const char* p = L->GetString(1, &n); L->PushBool(p && Parse(p, n)); return 1; }int DebugStateDef(ILuaBase* L) { if (!L->IsType(1, Type::Number))return 0; double x = L->GetNumber(1); if (!std::isfinite(x) || x != std::floor(x) || x < 0 || x>1048575)return 0; auto lock = LockForRead(); const StateDef* s = FindState(uint32_t(x)); if (!s)return 0; L->CreateTable(); L->PushNumber(x); L->SetField(-2, "stateId"); L->PushNumber(s->blockId); L->SetField(-2, "blockId"); L->PushNumber(s->orient); L->SetField(-2, "orient"); L->PushNumber(s->liquidKind); L->SetField(-2, "liquidKind"); L->PushNumber(s->liquidAmount); L->SetField(-2, "liquidAmount"); L->PushBool((s->flags & STATE_LEGACY_EXACT) != 0); L->SetField(-2, "legacyExact"); L->PushBool((s->flags & STATE_FULL_CUBE) != 0); L->SetField(-2, "fullCube"); L->PushBool((s->flags & STATE_CULL_GENERATED_FACES) != 0); L->SetField(-2, "cullGeneratedFaces"); L->PushBool((s->flags & STATE_USE_GENERATED_VISUAL) != 0); L->SetField(-2, "useGeneratedVisual"); const BlockDef* d = FindDef(s->blockId); const OrientDef* o = OrientEntry(d, s->orient); if (o) { L->PushNumber(uint8_t(o->emitter)); L->SetField(-2, "emitter"); L->PushNumber(o->pass); L->SetField(-2, "pass"); L->PushBool((o->flags & 1) != 0); L->SetField(-2, "fullCube"); }return 1; }
