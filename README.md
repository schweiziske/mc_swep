...
.
├─ adapter/
│  └─ lua/autorun/client/mcswep_native_bridge.lua
...
- `adapter/lua/autorun/client/mcswep_native_bridge.lua`：运行时适配器、MCBD打包、chunk snapshot上传、代理mesh和fallback。
...
- C++17；
- Garry's Mod客户端；
- [danielga/garrysmod_common](https://github.com/danielga/garrysmod_common)；
...
`garrysmod_common`使用兼容64位Source SDK的：

```text
x86-64-support-sourcesdk
```
...
如果本地依赖尚未初始化，应确保上述三个目录分别检出对应上游分支。不要用`master`版Source SDK替代x64目录，否则可能遇到旧SDK头文件、MMX intrinsic或库架构不匹配问题。
...
| 配置 | 输出文件名 |
|---|---|
| Release/Debug x64 | `gmcl_mcswep_native_mesh_win64.dll` |
| Release/Debug Win32 | `gmcl_mcswep_native_mesh_win32.dll` |
...
- bridge协议变化时必须同步更新adapter、DLL解析器、ABI/cache identity和接口文档。
